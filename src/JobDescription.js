...
class JobDescription extends React.Component {
  ...
  getRequirements = (job_desc) => {

    const doc = new DOMParser().parseFromString(job_desc, "text/html");
    const matches = [...doc.querySelectorAll("ul")].map(
      (style) => style.textContent
    );

    let requirements = matches[1].split("\n");
    requirements.shift();
    requirements.pop();
  ...
  getResponsibilities = (job_desc) => {
    ...
    let responsibilities = matches[0].split("\n");
  ...
  render() {
    ...
    return (
      <div>
        ...
        <h2>{company}</h2>
        <a href={jobInfo.job_link} target="_blank" rel="noopener noreferrer">
          Job Posting Link
        </a>
        ...
        <h1>Requirements:</h1>
        <ul>{this.getRequirements(jobInfo.job_description)}</ul>
        <h1>Responsibilities:</h1>
        <ul>{this.getResponsibilities(jobInfo.job_description)}</ul>
      </div>
    );
  }
}
